...
import { CLAWDESKTOP_DIR } from './constants.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, CLAWDESKTOP_DIR, 'base', resolvedPath);
  ...
  try {
    ...
    for (const relPath of manifest.modifies) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `clawdesktop-merge-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
      ...
      for (const cmd of manifest.post_apply) {
        try {
          ...
          return {
            ...
            skill: manifest.skill,
            version: manifest.version,
            error: `post_apply command failed: ${cmd} — ${errMsg}`,
          };
        }
      }
    }
