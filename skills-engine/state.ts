...
import type { AppliedSkill, CustomModification, SkillState } from './types.js';

function getStatePath(): string {
  return path.join(process.cwd(), CLAWDESKTOP_DIR, STATE_FILE);
}
...
export function readState(): SkillState {
  ...
  if (!fs.existsSync(statePath)) {
    throw new Error(
      '.clawdesktop/state.yaml not found. Run initSkillsSystem() first.',
    );
  }
