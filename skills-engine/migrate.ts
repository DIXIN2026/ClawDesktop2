...
import { BASE_DIR, CUSTOM_DIR, CLAWDESKTOP_DIR } from './constants.js';
import { initClawDesktopDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initClawDesktopDir();
  console.log('Skills system initialized. .clawdesktop/ directory created.');
}
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      const filesModified = [...diff.matchAll(/^diff -ruN .+ (.+)$/gm)]
        ...
        .filter((f) => !f.startsWith('.clawdesktop'));
      ...
      console.log(
        'Custom modifications captured in .clawdesktop/custom/migration.patch',
      );
    } else {
