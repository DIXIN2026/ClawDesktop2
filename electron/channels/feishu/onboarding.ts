...
import { addWildcardAllowFrom, DEFAULT_ACCOUNT_ID, formatDocsLink } from "./plugin-adapter.js";
...
function setFeishuDmPolicy(cfg: ClawdbotConfig, dmPolicy: DmPolicy): ClawdbotConfig {
  const allowFrom =
    dmPolicy === "open"
      ? addWildcardAllowFrom(cfg.channels?.feishu?.allowFrom as Array<string | number> | undefined)?.map((entry) => String(entry))
      : undefined;
  return {
    ...cfg,
    channels: {
...
async function promptFeishuAllowFrom(params: {
  ...
  while (true) {
    const existingEntries = (cfg.channels?.feishu as FeishuConfig | undefined)?.allowFrom ?? [];
    const entry = await params.prompter.text({
      message: "Feishu allowFrom (user open_ids)",
      placeholder: "ou_xxxxx, ou_yyyyy",
      initialValue: existingEntries[0] ? String(existingEntries[0]) : undefined,
    ...
    const unique = [
      ...new Set([
        ...existingEntries.map((v: string | number) => String(v).trim()).filter(Boolean),
        ...parts,
      ]),
    ];
    return setFeishuAllowFrom(cfg, unique);
  }
}
...
export const feishuOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter }) => {
    ...
    if (groupPolicy === "allowlist") {
      const existingGroups = (next.channels?.feishu as FeishuConfig | undefined)?.groupAllowFrom ?? [];
      const entry = await prompter.text({
        message: "Group chat allowlist (chat_ids)",
        placeholder: "oc_xxxxx, oc_yyyyy",
        initialValue: existingGroups.length > 0 ? existingGroups.map(String).join(", ") : undefined,
      });
      if (entry) {
        const parts = parseAllowFromInput(String(entry));
        if (parts.length > 0) {
