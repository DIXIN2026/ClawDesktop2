import type { ClawdbotConfig, RuntimeEnv } from "./plugin-adapter.js";
import {
...
async function resolveFeishuSenderName(params: {
  ...
  try {
    ...
    const userData = (res?.data as Record<string, unknown> | undefined)?.user as Record<string, unknown> | undefined;
    const name: string | undefined =
      (userData?.name as string) ||
      (userData?.display_name as string) ||
      (userData?.nickname as string) ||
      (userData?.en_name as string) ||
      undefined;
...
export async function handleFeishuMessage(params: {
  ...
  try {
    ...
    if (isGroup && ctx.rootId) {
      const groupCfg = resolveFeishuGroupConfig({ cfg: feishuCfg, groupId: ctx.chatId });
      topicSessionMode = groupCfg?.topicSessionMode ?? feishuCfg?.topicSessionMode ?? "disabled";
    ...
    if (!isGroup && route.matchedBy === "default") {
      ...
      if (dynamicCfg?.enabled) {
        const runtimeInstance = getFeishuRuntime();
    ...
    // Resolve media from message (use effectiveCfg which may have been updated by dynamic agent)
    ...
    // Include a readable speaker label
