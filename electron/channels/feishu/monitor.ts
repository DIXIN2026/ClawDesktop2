...
async function monitorSingleAccount(params: MonitorAccountParams): Promise<void> {
  const { cfg, account, runtime, abortSignal: _abortSignal } = params;
...
async function monitorWebhook({
  ...
  log(`feishu[${accountId}]: starting Webhook server on ${host}:${port}, path ${webhookPath}...`);

  const server = http.createServer();
  const webhookHandler = Lark.adaptDefault(webhookPath, eventDispatcher, { autoChallenge: true });
  server.on("request", (req, res) => {
    res.on("finish", () => {
      recordWebhookStatus(runtime, accountId, webhookPath, res.statusCode);
    });

    const rateLimitKey = `${accountId}:${webhookPath}:${req.socket.remoteAddress ?? "unknown"}`;
