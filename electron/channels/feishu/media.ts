...
import { withTempDownloadPath, type ClawdbotConfig } from "./plugin-adapter.js";
...
async function readFeishuResponseBuffer(params: {
  ...
  if (responseAny.data && Buffer.isBuffer(responseAny.data)) {
    return responseAny.data as Buffer;
  }
  if (responseAny.data instanceof ArrayBuffer) {
    return Buffer.from(responseAny.data);
  }
  if (typeof (responseAny as Record<string, unknown>).getReadableStream === "function") {
    const stream = (responseAny as { getReadableStream: () => AsyncIterable<Buffer> }).getReadableStream();
    const chunks: Buffer[] = [];
  ...
  if (typeof (responseAny as Record<string, unknown>).writeFile === "function") {
    return await withTempDownloadPath({ prefix: params.tmpDirPrefix }, async (tmpPath) => {
      await (responseAny as { writeFile: (p: string) => Promise<void> }).writeFile(tmpPath);
      return await fs.promises.readFile(tmpPath);
    });
  }
  if (typeof (responseAny as Record<string, unknown>)[Symbol.asyncIterator as unknown as string] === "function") {
    const chunks: Buffer[] = [];
    for await (const chunk of responseAny as unknown as AsyncIterable<Buffer>) {
      chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk));
    }
    return Buffer.concat(chunks);
  }
  if (typeof (responseAny as Record<string, unknown>).read === "function") {
    const chunks: Buffer[] = [];
    for await (const chunk of responseAny as unknown as Readable) {
      chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk));
    }
    return Buffer.concat(chunks);
  }
...
export async function uploadImageFeishu(params: {
  ...
  if (responseObj.code !== undefined && responseObj.code !== 0) {
    throw new Error(`Feishu image upload failed: ${responseObj.msg || `code ${responseObj.code}`}`);
  }

  const imageKey = (responseObj.image_key ?? (responseObj.data as Record<string, unknown> | undefined)?.image_key) as string | undefined;
  if (!imageKey) {
...
export async function uploadFileFeishu(params: {
  ...
  const responseObj = response as Record<string, unknown>;
  if (responseObj.code !== undefined && responseObj.code !== 0) {
    throw new Error(`Feishu file upload failed: ${responseObj.msg || `code ${responseObj.code}`}`);
  }

  const fileKey = (responseObj.file_key ?? (responseObj.data as Record<string, unknown> | undefined)?.file_key) as string | undefined;
  if (!fileKey) {
